...
TEST_CASE (
	...
	const auto xcollection = collection(xs);
	...
	uint32_t index = 0;
	for(const uint32_t y : collection(xs).map(square).collect())
	...
	for(const uint32_t y : collection(xs).filter(is_even).collect())
	...
	index = 0;
	for(const uint32_t y : collection(xs).map(square).filter(is_even).collect())
		CHECK( y == xevenSquares[index++]);

	index = 0;
	for(const uint32_t y : collection(xs).filter(is_even).map(square).collect())
		CHECK( y == xevenSquares[index++]);

	//flatMap
	index = 0;
	for(const uint32_t y : collection(xevens).flatMap(
			[] (const int x) -> std::vector<int> {return {x, x + 1}; }
		).collect())
		CHECK( y == xs[index++]);

	index = 0;
	for(const uint32_t y : collection(xs).filter(is_even
		).flatMap(
			[] (const int x) -> std::vector<int> {return {x, x + 1};}
		).collect())  {
		CHECK( y == xs[index++]);
	...
	index = 0;
	for(const uint32_t y : collection(xs).filter(is_odd
		).flatMap(
...
)
{
	...
	const auto xcollection = view(xs);
