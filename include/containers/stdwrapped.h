...
template<typename T>
inline const std::wrapped::List<T>& operator >> (const T& head,
												 std::wrapped::List<T> tail)
{
	tail.push_front(head);
...
>
inline std::wrapped::List<S>& operator >>= (std::wrapped::List<T>& ts,
											const F& fst)
{
	return flatten( map(fst, ts));
}
template<typename T>
inline bool operator==(const std::wrapped::List<T>& l1,
					   const std::wrapped::List<T>& l2)
{
	if (l1.empty()) return l2.empty();
	if (l2.empty()) return false;
	return (std::wrapped::head(l1) == std::wrapped::head(l2) and
			std::wrapped::tail(l1) == std::wrapped::tail(l2));
}
