...
#include <list>
namespace fxpp {namespace wrapped {
...
template<typename T>
inline fxpp::wrapped::List<T>& operator >> (
  const T& head,
  fxpp::wrapped::List<T>& tail)
{
...
template<typename T>
inline const fxpp::wrapped::List<T>&
operator >> (
  const T& head,
  fxpp::wrapped::List<T> tail)
{
...
>
inline fxpp::wrapped::List<S>&
operator >>= (
  fxpp::wrapped::List<T>& ts,
  const F& fst)
{
...
inline bool operator==(
  const fxpp::wrapped::List<T>& l1,
  const fxpp::wrapped::List<T>& l2)
{
	if (l1.empty()) return l2.empty();
	if (l2.empty()) return false;
	return
    fxpp::wrapped::head(l1) == fxpp::wrapped::head(l2) and
    fxpp::wrapped::tail(l1) == fxpp::wrapped::tail(l2);
}
