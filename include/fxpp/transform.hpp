...
namespace fxpp { namespace transform
{
template<
	typename in_type,
...
struct MapTransform
{
	...
	static container<out_type> transform(
...
struct FilterTransform
{
	...
	static container<in_type> transform(
		const function& pred,
		const container<in_type>& xs)
	{
...
struct FlatMapTransform
{
	using function = std::function<bool(const in_type&)>;
	static container<out_type> transform(
		const function& fmapped,
		const container<in_type>& xs)
	{
		container<out_type> ys;
		for (const in_type& : xs) {
...
public:
	...
	const PrevViewType& previously() const
	{return 
		_previously;}
	const typename collection_policy::function& transformer() const
	{return
		_transformer;}
	size_t size() const
	{return
	...
	{return
		collection_policy::transform(
			_transformer,
			_previously.collect());}
	template<
		typename Mapper,
		typename R = typename std::result_of<
			Mapper&(mapped_type)>::type>
	NextMapHeadedView map(
		const Mapper& nextMapper) const
	{return
		NextMapHeadedView<R>(
			*this,
			nextMapper);}
	template<
		typename Predicate>
	NextFilterHeadedView filter(
		const Predicate& pred) const
	{return
		NextFilterHeadedView(
			*this,
			pred);}
	template<
		typename FlatMapper,
		typename CR = typename std::result_of<
			FlatMapper&(mapped_type)>::type,
		typename R  = typename CR::value_type>
	NextFlatMapHeadedView<R> flatMap(
		const FlatMapper& flatMapper) const
	{return
		NextFlatMapHeadedView<R>(
			*this,
			flatMapper);}
private:
...
template<
	...
	typename T>
NillView<C, T> view(const C<T>& xs)
{return
	NillView<C, T>(xs);}
} /*namespace transform*/ } /*namespace fxpp*/
